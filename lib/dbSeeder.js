    ...
    seed() {
        ...
        for (i = 0; i < l; i++) {
            ...
            state.save();
               ...
               var productNames =
               [
                   "Chocolate Cake,Sugar Mold (Fondant),8 Inch,15",
                   "Vanilla Cake, Sugar Mold (Fondant), 9 Inch, 20"

               ];
               var productSpecs =
               [
                   "Covered with sugar paste under which are 4 layers of cake filled with cream and coated with chocolate ganache",
                   "Covered with sugar paste under which are 4 layers of cake filled with cream and coated with butter cream"
               ];
               var category =
               [
                   "Cakes",
               ...
               var l = productNames.length,
                   i;

               for (i = 0; i < l; i++) {
                   var productName = productNames[i].split(',');
                   var product = new Product({
                       'productName': productName[0],
                       'productCovering': productName[1],
                       'productDimensions': productName[2],
                       'productServings': productName[3],
                       'description': productSpecs[i],
                       'category': category[i],
                       'imageUrl': productUrl[i]
                   });

                   product.save((err, cust) => {
                     if (err) {
                         console.log(err);
                     } else {
                         console.log('inserted product: ' + cust.productName);
                     }
                 });
               ...
               [
                  'Sam, Will, SamWill@gmail.com, 00961222333',
                  'Jane, Smith, JaneSmith@gmail.com, 00961333444'
               ];
               ...
               var orderSpecs =
               [
                 '15, Birthday, SuperMan',
                 '25, Engagment, Flowers'
               ];

               Order.remove({});

               var ordersLength = customerIdentification.length;

               for(i = 0; i < ordersLength; i++) {
                  var idLength = customerIdentification[i].split(',');
                  var typeLength = orderType[i].split(',');
                  var specsLength = orderSpecs[i].split(',');
                  var orders = new Order ({
                    'firstName': idLength[0],
                    'lastName': idLength[1],
                    'email': idLength[2],
                    'phone': idLength[3],
                    'orderType1': typeLength[0],
                    'orderType2': typeLength[1],
                    'orderType3': typeLength[2],
                    'orderType4': typeLength[3],
                    'servingsList': specsLength[0],
                    'occassion': specsLength[1],
                    'theme': specsLength[2]
                  ...
                  orders.save((err, ord) => {
                    if(err){
                      ...
                      console.log('inserted Product ' + ord.firstName + ' ' + ord.lastName);
                    }
                  });
               }
    }
